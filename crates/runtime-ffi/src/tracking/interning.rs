...
// Since [`svm_byte_array`](crate::svm_byte_array) is exchanged between `Rust` and external-code (i.e FFI code)
// we can't use [`Type`] as the `type_id`.
//
// That's why we apply the interning technique for [`Type`].
// By doing that we can use its interned representation within [`svm_byte_array`](crate::svm_byte_array).
//
