...
fn contract_storage_loading_an_empty_slice_into_the_cache() {
    ...
    let (_addr, _kv, mut storage) = contract_storage_init(addr, pages_count);
...
#[test]
fn contract_storage_read_an_empty_slice_then_override_it_and_then_commit() {
    let addr = 0x11_22_33_44;
    let pages_count = 10;
    ...
    let ph = default_page_hash(&addr, 0, &[10, 20, 30]);
    assert_no_key!(&kv, ph.0);
}

#[test]
fn contract_storage_write_slice_without_loading_it_first_and_commit() {
    let addr = 0x11_22_33_44;
    let pages_count = 2;
    ...
    contract_storage_open(&addr, &new_state, &kv, pages_count);
...
#[test]
fn contract_storage_read_an_existing_slice_then_overriding_it_and_commit() {
    let addr = 0x11_22_33_44;
    let pages_count = 2;
...
#[test]
fn contract_storage_write_slice_and_commit_then_load_it_override_it_and_commit() {
    let addr = 0x11_22_33_44;
    let pages_count = 2;
    ...
    let mut storage = contract_storage_open(&addr, &state, &kv, pages_count);
    assert_eq!(vec![10, 20, 30], storage.read_page_slice(&layout));
...
#[test]
fn contract_storage_write_two_slices_under_same_page_and_commit() {
    let addr = 0x11_22_33_44;
    let pages_count = 2;
