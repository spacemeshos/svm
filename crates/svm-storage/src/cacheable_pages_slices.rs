...
impl<'ps, PS: PagesStorage> CacheablePagesSlices<'ps, PS> {
    ...
    pub fn read_page_slice(&mut self, page_idx: u32, offset: u32, len: u32) -> Option<Vec<u8>> {
        None
    }

    /// * we insert the new page slice into `cached_slices` as `Cached(PageSlice)` and mark it as dirty
    pub fn write_page_slice(&mut self, page_idx: u32, offset: u32, len: u32, data: Vec<u8>) {
        //
    }
