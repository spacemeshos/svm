...
impl Storage {
    ...
    pub fn set32(&self, var_id: u32, value: u32) {
        let mut storage = storage();

        storage.set32(var_id, value)
    }

    pub fn set64(&self, var_id: u32, value: u64) {
        let mut storage = storage();

        storage.set64(var_id, value)
    }

    pub fn store160(&self, var_id: u32, ptr: usize) {
        let mut storage = storage();

        storage.store160(var_id, ptr)
    }

    pub fn store256(&self, var_id: u32, ptr: usize) {
        let mut storage = storage();
    ...
    pub fn from_raw_parts(&self, ptr: usize, len: usize) -> &[u8] {
        unsafe { core::slice::from_raw_parts(ptr as *const u8, len) }
    }
}
