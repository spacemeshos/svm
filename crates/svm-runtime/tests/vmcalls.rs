...
fn prepare_test_args() -> (
    ...
    let host_ctx = DataWrapper::new(Box::new(HashMap::new()));
    let pages_count = 5;

    (addr, state, host, host_ctx, pages_count)
}
...
fn vmcalls_mem_to_reg_copy() {
    let (addr, state, host, host_ctx, pages_count) = prepare_test_args();

    let import_object = imports! {
        move || testing::app_memory_state_creator(addr, state, host, host_ctx, pages_count),
