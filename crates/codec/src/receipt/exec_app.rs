...
pub fn encode_exec_receipt(receipt: &TxReceipt) -> Vec<u8> {
    let mut w = Vec::new();
...
pub fn decode_exec_receipt(bytes: &[u8]) -> TxReceipt {
...
fn encode_new_state(receipt: &TxReceipt, w: &mut Vec<u8>) {
...
fn encode_returndata(receipt: &TxReceipt, w: &mut Vec<u8>) {
    debug_assert!(receipt.success);
