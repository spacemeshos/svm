...
/// Illustration for handling an `if-then` (without `else`):
///
/// +----------------------+
/// |       block          | depth = `d`
/// +----------------------+      
...
///        \/   \/
/// +-----------------------------------+
/// |       `after-if` block `D`        | depth = `d`
/// +-----------------------------------+
///
///
/// Illustration for handling an `if-then-else`:
///
/// +----------------------+
/// |       block          | depth = `d`
/// +----------------------+      
...
impl<'f> CFGBuilder<'f> {
    ...
    /// Entering `If`:
    ///
    /// 1. Add a new unresolved-cont of kind: `after-if`   (under the `current depth`) with `origin = current block`
    /// 2. Create a new block and make it the `current block`
    /// 3. add a continuation from the previous `current scope` to the new one (created at 2.)
    /// 4. Call `enter_scope`
    ...
    /// 1. Add a new unresolved-cont of kind: `after-else` (under the `current depth`) with `origin = current block`
    /// 2. Create a new block and make it the `current block`
    /// 3. Add a continuation from the previous `current scope` to the new one (created at 2.)
    /// 4. Call `enter_scope`
    ...
    // 1. Append `op` to the current block
    // 2. Decrement the `current depth`
    // 3. If there are no unresolved-jumps associated with `current depth`:
    //    3.1 Finish
    // 4. Else
    //    4.1 Create a new block and make it the `current block`
    //    4.2 Resolve each `unresolved jump` associated with the `current depth`
